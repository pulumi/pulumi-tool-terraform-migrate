...
func TestMarshallableResourceShimSchemaType(t *testing.T) {
	...
	t.Run("plain attributes", func(t *testing.T) {
		...
		marshalledResource := info.MarshalResourceShim(resource.Shim())
		jsonBlob, err := json.Marshal(marshalledResource)
		if err != nil {
			t.Fatalf("failed to marshal marshalledResource: %v", err)
		}

		unmarshallableResource := MarshallableResourceShimWithTypes{}
		err = json.Unmarshal(jsonBlob, &unmarshallableResource)
		if err != nil {
			t.Fatalf("failed to unmarshal marshalledResource: %v", err)
		}
		unmarshalledResource := unmarshallableResource.Unmarshal()
	...
	t.Run("collections", func(t *testing.T) {
		...
		err = json.Unmarshal(jsonBlob, &unmarshallableResource)
		if err != nil {
			t.Fatalf("failed to unmarshal unmarshalledResource: %v", err)
		}
		unmarshalledResource := unmarshallableResource.Unmarshal()
