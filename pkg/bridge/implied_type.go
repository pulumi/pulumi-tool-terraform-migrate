...
//
// This is necessary in order to recover the [cty.Type] of a resource in a marshalled pulumi provider.
...
func impliedTypeForSchema(schema shim.Schema) cty.Type {
	...
	if isSchemaElem {
		schemaElem := schema.Elem().(shim.Schema)
		if schemaHasDynamicTypes(schemaElem) {
			return cty.DynamicPseudoType
		}
	...
	if isResElem {
		resElem := schema.Elem().(shim.Resource)
		if resourceHasDynamicTypes(resElem) {
			return cty.DynamicPseudoType
		}
